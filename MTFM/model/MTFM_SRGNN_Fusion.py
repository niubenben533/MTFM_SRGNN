...
"""
MTFM + SRGNN 融合模型
将SRGNN的会话图嵌入与MTFM的语义和特征交互嵌入融合
cd ~/learn/MTFM-main
python -m model.MTFM_SRGNN_Fusion
"""
import sys
import os
import math
...
class FusionMTFMConfig(object):
    def __init__(self, ds_config):
        ...
        # 融合权重
        self.fusion_mode = 'concat'  # 'concat' or 'weighted'
        self.use_srgnn = True  # 是否使用SRGNN组件


# ==================== 融合模型 ====================
class MTFM_SRGNN_Fusion(nn.Module):
    def __init__(self, config):
        super(MTFM_SRGNN_Fusion, self).__init__()
        self.config = config
        ...
        self.fic_api_feature_embedding = nn.Parameter(
            torch.rand(config.feature_dim, config.num_api))
    ...
    def forward(self, mashup_des, api_seq=None, api_adj=None, api_mask=None):
        """
        Args:
            mashup_des: [batch_size, max_doc_len] Mashup文本描述
            api_seq: [batch_size, max_seq_len] API序列 (可选)
        ...
        if self.config.use_srgnn and api_seq is not None:
            # 获取会话嵌入
            session_emb = self.session_encoder(api_seq, api_adj, api_mask)
            # 投影到num_api维度
            u_srgnn = self.srgnn_projection(session_emb)  # [batch_size, num_api]
            u_srgnn = self.tanh(u_srgnn)
...
def collate_fn_with_graph(batch, num_api):
    ...
    Args:
        ...
        num_api: API总数
    Returns:
        batch data with graph structures
    ...
    for used_api in used_apis:
        ...
        if api_indices.dim() == 0:  # 单个API
            ...
            api_ids = [idx.item() + 1 for idx in api_indices]  # +1因为SRGNN中0是padding
...
# ==================== 训练类 ====================
class FusionTrain(object):
    def __init__(self, input_model, input_config, train_iter, test_iter, val_iter, 
                 case_iter, log, input_ds, model_path=None):
        ...
        if model_path:
            ...
            self.model_path = os.path.join(project_root, 'checkpoint', 
    ...
    def evaluate(self, test=False):
        ...
        with torch.no_grad():
            for batch_idx, batch_data in enumerate(data_iter):
                ...
                ndcg_, recall_, ap_, pre_ = metric(api_target, api_pred, 
    ...
    def case_analysis(self):
        ...
        case_path = os.path.join(case_dir, f'{self.config.model_name}.json')
        ...
        with torch.no_grad():
            for batch_idx, batch_data in enumerate(self.case_iter):
                index, des, category, api_target_batch, items, A, mask = batch_data
                ...
                api_target = []
                for api_data in api_target_batch:
                ...
                if self.config.use_srgnn:
                    api_pred_ = self.model(des, items, A, mask)
                else:
...
if __name__ == '__main__':
    ...
    model = MTFM_SRGNN_Fusion(config)
